...
export default function PromptTestTopSection() {
  ...
  const maxRowTotal = Math.max(...BAR_CHART_DATA.map((d) => d.values.reduce((a, b) => a + b, 0)));
          ...
          <div>
            <span className="flex items-center gap-1 text-sm text-muted-foreground mb-1">
              <TextMorph
                ...
                as="span"
              >
                {hoveredMonth ? hoveredMonth.label : "Total"}
              </TextMorph>
              Revenue
              {!hoveredMonth && (
          ...
          <div className="grid grid-cols-2 gap-4">
            {REVENUE_CATEGORIES.map((category, idx) => (
              <div key={category.id} className="flex items-center gap-3">
                ...
                />
                <div>
                  <span className="block text-xs mb-1 text-muted-foreground">{category.label}</span>
          ...
          >
            {BAR_CHART_DATA.map((item) => {
              ...
              return (
                ...
                >
                  <span
                    className="text-xs text-muted-foreground w-8 shrink-0 truncate"
                    title={item.label}
                  >
