...
const NumberColumn = memo(({ value }: { value: number }) => {
  ...
  const activeTransform = `translateY(calc(${value} * -1.2em))`;
      ...
      <div
        ...
        style={{
          ...
          transition: isInitialMount.current
            ? "none"
            : `transform ${ANIMATION_DURATION}ms cubic-bezier(.02,1.02,.38,.93)`,
        }}
      >
...
const REVENUE_CATEGORIES = [
  { id: "product", label: "Product Sales", value: 198000, color: "bg-[#14b8a6]" },
  { id: "subscription", label: "Subscriptions", value: 142500, color: "bg-[#6ecddb]" },
  { id: "services", label: "Services", value: 72000, color: "bg-[#a78bfa]" },
  { id: "licensing", label: "Licensing", value: 37500, color: "bg-[#6366f1]" },
] as const;
...
export default function PromptTestTopSection() {
  ...
  const totalValue = hoveredMonth
    ? hoveredMonth.values.reduce((a, b) => a + b, 0) * 1000
    : 450000;
  const categoryValues = hoveredMonth
    ? hoveredMonth.values.map((v) => v * 1000)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2">
        <div className="flex flex-col gap-6 pr-6 lg:border-r lg:border-border">
          <div>
            ...
            <NumberScroller
              value={totalValue}
              className="text-6xl font-semibold text-card-foreground"
