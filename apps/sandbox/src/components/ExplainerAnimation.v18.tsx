...
function OnScreenText({ text, visible }: { text: string; visible: boolean }) {
  ...
  return (
    <motion.div
      key={text}
      className="absolute left-8 top-1/2 -translate-y-1/2 z-50 pointer-events-none flex flex-wrap gap-x-2"
      initial="hidden"
    ...
    >
      {words.map((word, index) => (
        <motion.span
          key={`${text}-${index}-${word}`}
          className="text-foreground text-5xl font-medium tracking-tight inline-block"
          variants={{
            hidden: {
              opacity: 0,
              y: 30,
...
const ANIMATION_TIMELINE: TimelineAction[] = [
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // 5. Toggle between versions - Slowly toggle in order v1 → v2 → v3 → v4 → v5 → v6
  //    Let each version sit for ~0.5-0.7s
  // ─────────────────────────────────────────────────────────────────────────────
  ...
  { type: "wait", duration: 600 }, // Let it sit
  ...
  // v5
  { type: "moveTo", target: '[data-version="v5"]', duration: 400 },
  { type: "hover", version: "v5" },
  { type: "wait", duration: 200 },
  { type: "click" },
  {
  ...
  // v6
  { type: "moveTo", target: '[data-version="v6"]', duration: 400 },
