...
2. The \`uifork watch\` command will automatically update \`versions.ts\`
...
Use the \`uifork watch\` command to automatically sync version files:

\`\`\`bash
uifork watch ${this.componentName}
\`\`\`
...
`;
  ...
  scaffold() {
    ...
    console.log(
      `3. Run: uifork watch ${this.componentName} (to watch for changes)`
    );
