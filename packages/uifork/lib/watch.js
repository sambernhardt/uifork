...
class ComponentManager {
  constructor(versionsFilePath, options = {}) {
    ...
    // "default" or a named export identifier (e.g. "Button")
    this.exportName = this.parseExportName();
  }
    ...
    if (this.lazy) {
      const lazyImports = versionFiles
        .map((file) => {
          ...
          if (isNamed) {
            return `const ${importName} = lazy(() => import("${filePath}").then(m => ({ default: m.${this.exportName} })));`;
          }
      ...
      imports = versionFiles
        .map((file) => {
          ...
          const filePath = `./${file.replace(/\.(tsx?|jsx?)$/, "")}`;
          if (isNamed) {
            return `import { ${this.exportName} as ${importName} } from "${filePath}"`;
          }
